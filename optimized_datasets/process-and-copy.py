...
def extract_features(f):
    ...
    if songs is not None:
        if isinstance(songs, np.ndarray) and songs.dtype.names:
            ...
            features['key'] = int(songs['key'][0]) if 'key' in songs.dtype.names else 0
            features['mode'] = int(songs['mode'][0]) if 'mode' in songs.dtype.names else 0
            features['time_signature'] = int(songs['time_signature'][0]) if 'time_signature' in songs.dtype.names else 4
        else:
            ...
            features['time_signature'] = int(safe_get(f, 'analysis/songs/time_signature', 4))

        features['rhythmic_complexity'] = features['tempo'] * features['time_signature'] / 120
    ...
    if segments_timbre is not None:
        ...
        if timbre.ndim == 2 and timbre.shape[0] > 0:
            timbre_normalized = timbre / (np.sum(np.abs(timbre), axis=1, keepdims=True) + 1e-10)
            ...
            features['timbre_skew'] = np.nanmean(skew(timbre, axis=0, nan_policy='omit'))
            features['timbre_kurtosis'] = np.nanmean(kurtosis(timbre, axis=0, nan_policy='omit'))

            n_fft = min(2048, timbre.shape[0])
            features['spec_bw'] = np.nanmean(spectral_bandwidth(y=timbre.T, sr=22050, n_fft=n_fft)[0])
            ...
            features['timbre_range'] = features['timbre_max'] - features['timbre_min']
            features['timbre_median'] = np.nanmedian(timbre, axis=0).mean()
            features['timbre_variance'] = np.nanmean(np.nanvar(timbre, axis=0))
            features['timbre_mad'] = np.nanmean(np.abs(np.diff(timbre, axis=0)))
            timbre_sum = np.nansum(np.nanmean(timbre, axis=1))
            features['timbre_temporal_centroid'] = np.nanmean(np.arange(len(timbre)) * np.nanmean(timbre, axis=1)) / (timbre_sum + 1e-10)

            features['timbre_q1'] = np.nanpercentile(timbre, 25, axis=0).mean()
            features['timbre_q3'] = np.nanpercentile(timbre, 75, axis=0).mean()
            features['timbre_iqr'] = features['timbre_q3'] - features['timbre_q1']
            features['timbre_entropy'] = entropy(np.abs(timbre_normalized) + 1e-10, axis=0).mean()
            features['timbre_energy'] = np.nansum(timbre**2, axis=0).mean()
            features['timbre_flux'] = np.nanmean(np.diff(timbre, axis=0)**2)
            features['timbre_flatness'] = gmean(np.abs(timbre) + 1e-10, axis=0).mean() / (np.nanmean(np.abs(timbre), axis=0).mean() + 1e-10)
            ...
            features['brightness'] = np.nanmean(np.sum(timbre[:, 1:], axis=1) / (np.sum(timbre, axis=1) + 1e-10))
    ...
    if segments_loudness is not None:
        ...
        features['loudness_range'] = features['loudness_max'] - features['loudness_min']
        ...
        features['loudness_iqr'] = features['loudness_q3'] - features['loudness_q1']
    ...
    if segments_pitches is not None:
        ...
        features['chroma_skew'] = np.nanmean(skew(pitches, axis=0, nan_policy='omit'))
        features['chroma_kurtosis'] = np.nanmean(kurtosis(pitches, axis=0, nan_policy='omit'))
        features['chroma_max'] = np.max(np.mean(pitches, axis=0))
        features['chroma_min'] = np.min(np.mean(pitches, axis=0))
        features['chroma_range'] = features['chroma_max'] - features['chroma_min']
        features['chroma_median'] = np.nanmedian(pitches, axis=0).mean()

        pitch_diff = np.diff(pitches, axis=0)
        features['zcr'] = np.nanmean(np.sum(np.abs(np.sign(pitch_diff)), axis=1) / (2 * pitches.shape[1]))
        ...
        features['chroma_flatness'] = gmean(pitches + 1e-10, axis=0).mean() / (np.nanmean(pitches + 1e-10, axis=0).mean())

        melodic_contour = np.argmax(pitches, axis=1)
        features['melodic_contour_direction'] = np.nanmean(np.diff(melodic_contour))
        features['melodic_contour_interval'] = np.nanmean(np.abs(np.diff(melodic_contour)))
...
def process_and_copy_h5(input_files, output_dir, unified_df=None):
    ...
    for filename in tqdm(files_to_process, desc="Processing files"):
        ...
        input_file = next((f for f in input_files if clean_filename(f) == clean_name), None)
        if input_file:
            ...
            with h5py.File(output_file, 'r+') as f:
                genre = unified_df.loc[unified_df['filename'] == filename, 'genre'].iloc[0] if unified_df is not None else f.attrs.get('genre')
                f.attrs['genre'] = genre
...
if __name__ == "__main__":
    ...
    # Filter genres with at least 70 tracks
    combined_summary = process_and_copy_h5(all_files, './processed_h5', unified_df)

    # Filter genres with at least 70 tracks
    balanced_summary = balance_dataset(combined_summary)

    # Preprocess numeric columns
    numeric_columns = balanced_summary.select_dtypes(include=[np.number]).columns
    non_numeric_columns = balanced_summary.select_dtypes(exclude=[np.number]).columns

    balanced_summary[numeric_columns] = balanced_summary[numeric_columns].abs()
    balanced_summary[numeric_columns] = balanced_summary[numeric_columns].replace([np.inf, -np.inf], np.nan)
    balanced_summary[numeric_columns] = balanced_summary[numeric_columns].fillna(balanced_summary[numeric_columns].median())

    # Define feature columns
    feature_columns = [col for col in balanced_summary.columns if col not in ['filename', 'genre', 'duration', 'key', 'mode', 'time_signature']]

    # Clip extreme values
    for column in feature_columns:
        if column in numeric_columns:
            balanced_summary[column] = balanced_summary[column].clip(-1e15, 1e15)

    # Scale features
    scaler = StandardScaler()
    balanced_summary[feature_columns] = scaler.fit_transform(balanced_summary[feature_columns])

    # Prepare features and labels
    X = balanced_summary[['duration', 'rhythmic_complexity'] + feature_columns + ['key', 'mode', 'time_signature']]
    ...
    print(f"Number of genres in the final dataset: {balanced_summary['genre'].nunique()}")
